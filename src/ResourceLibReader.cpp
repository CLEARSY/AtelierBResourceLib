...
void AtelierB::ResourceFileReader::loadFile(const fspath& path) {
  ...
  while (std::getline(file, line)) {
    ...
    // Replace variables
    {
      ...
      while (
          std::regex_search(searchStart, line.cend(), matches, variableRegex)) {
        ...
        if (replacement == NULL) {
          std::cerr << path.string() << ":" << lineNumber << ": "
...
const string& AtelierB::ResourceFileReader::lookupResource(const string& tool,
  ...
  for (size_t i = 0; i < m_resources.size(); ++i) {
    if (m_resources.at(i).m_tool == tool && m_resources.at(i).m_name == name) {
